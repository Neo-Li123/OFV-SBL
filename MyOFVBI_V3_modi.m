...
% we consider re-initialized the gamma before the refinement.
function [x_est,Result_est]=MyOFVBI_V3_modi(y,A,h,grid_size,bits,Para,Grid,K,Loc_tar,varargin)
% Input:
...
for ii=1:GridRef_iters
    %% ===============EM procedure===========================================
    [gamma,Mean_x,Sigma,Sigma_y,dmx]=EM(A_multi,gamma,y,h,var_n,bits);
    %% ------------------grid refine--------------------------------------
    ...
    if ~isempty(Loc_add_cur)        
        ...
        if any(gamma_grad<1e-2*gamma)% 如果存在梯度远小于能量
            ...
            if cluster_num>0
                ...
                Ind_delete=Ind_soft_full(idx_soft>0);
                gamma(Ind_delete)=[];
...
for mm=1:max_iters
    %===============EM procedure===========================================
    [gamma,Mean_x,Sigma,~,dmx]=EM(A_multi,gamma,y,h,var_n,bits);
    %===================Judge Convergence===================================
    if PRINT
    ...
    %-------------------------------Find centroid----------------------
        ...
        if ~isempty(Ind_neigh)% 如果存在与峰值很相近的元素
            ...
            idx_soft=dbscan(Grid_soft,1.5*min_interval,2);% 利用DBSCAN算法来找到簇
            cluster_num=max(idx_soft);            
            if cluster_num>0
                gamma_cent=[];
                Grid_cent=[];
                ...
                Ind_delete=Ind_neigh(idx_soft>0);
                gamma(Ind_delete)=[];
                gamma=[gamma;gamma_cent];
                Grid_2D(Ind_delete,:)=[];
                Grid_2D=[Grid_2D;Grid_cent];
                Grid_cent_mtr=[Grid_cent_mtr;Grid_cent];
                fprintf('Second Clusters.\n');
            end            
...
%% 网格化搜索+进一步更新幅度
    ...
    for kk=1:K
        ...
        gamma_kk=gamma(idx_kk);
        Ind_repeat=find(gamma==gamma_kk);
        Ind_repeat(Ind_repeat==idx_kk)=[];
        Loc_repeat=Grid_2D(Ind_repeat,:);
        Mse_repeat=sqrt(sum(abs(Loc_kk-Loc_repeat).^2,2));
        if ~isempty(Ind_repeat)&&max(Mse_repeat)<2*min_interval
            Ind_neigh_kk=Ind_repeat;
            idx_del=[Ind_neigh_kk,idx_kk];
            region_near_kk=[Grid_2D(idx_del,:)];
        else
            [region_near_kk,Ind_near_kk]=FindNeigh(Loc_kk,Grid_2D);
            Mse_near=sqrt(sum(abs(Loc_kk-region_near_kk).^2,2));
            region_near_kk(Mse_near>1.5*min_interval,:)=[];
            Ind_near_kk(Mse_near>1.5*min_interval)=[];
            if length(Ind_near_kk)<=1
                continue;
            end
            [~,Ind_sort]=sort(gamma(Ind_near_kk),'descend');
            idx_del=[idx_kk;Ind_near_kk(Ind_sort(1:num_neigh_del))];
        end
        region_near_kk=[region_near_kk;Loc_kk];
